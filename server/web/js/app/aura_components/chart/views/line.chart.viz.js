...
function(ViewAxialChart, d3, constants, Color)
{
	return ViewAxialChart.extend(
	{
		...
		updateScales: function()
		{
			...
			self.xScale.domain(d3.extent(this.data, function(d) { return d[self.getXScaleColumn()]; }));

			yDomain = [
				Math.min(0, d3.min(this.data, function(d){return d[self.getYScaleColumn()];})),
				d3.max(this.data, function(d){return d[self.getYScaleColumn()];})
			];
